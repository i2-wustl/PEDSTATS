...
void RewritePedigree()
   ...
   MarkerInfo ** markerInfo = new MarkerInfo * [ped.markerCount];
   for (int i = 0; i < ped.markerCount; i++)
       markerInfo[i] = ped.GetMarkerInfo(i);
   ...
   for (int f = 0; f < ped.familyCount; f++)
      ...
      for (int i = ped.families[f]->first; i <= ped.families[f]->last; i++)
         ...
         ped.WriteRecodedPerson(pedfile, i, markerInfo, 
                         (const char *) (ped[i].famid + "_" + groups[ped[i].traverse]), orig_id);
         }
      }
